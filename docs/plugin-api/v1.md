...
`submit` and `dynamic` routes require `decode` and `render`. `query` routes require `render`, prohibit `decode`, and use `taskIdParam` (default `task_id`). `proxy` routes are synchronous native HTTP proxies: they require `decode` and `render`, bypass task persistence and billing, and use the exported `buildProxyRequest` descriptor hook. Names refer only to callable members of `native`. Route conflicts use method plus normalized path shape and the route index is published atomically with its plugin generation.
